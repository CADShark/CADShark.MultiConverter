...
internal class SwSaveOperation
{
    internal static string ParseSaveError(swFileSaveError_e err)
    {
        ...
        if (err.HasFlag(swFileSaveError_e.swFileLockError)) errors.Add("File lock error");

        if (err.HasFlag(swFileSaveError_e.swFileNameContainsAtSign))
            errors.Add("File name cannot contain the at symbol(@)");

        if (err.HasFlag(swFileSaveError_e.swFileNameEmpty)) errors.Add("File name cannot be empty");

        if (err.HasFlag(swFileSaveError_e.swFileSaveAsBadEDrawingsVersion)) errors.Add("Bad eDrawings data");

        if (err.HasFlag(swFileSaveError_e.swFileSaveAsDoNotOverwrite)) errors.Add("Cannot overwrite an existing file");
        ...
        if (err.HasFlag(swFileSaveError_e.swFileSaveFormatNotAvailable)) errors.Add("Save As file type is not valid");
        ...
        if (err.HasFlag(swFileSaveError_e.swGenericSaveError)) errors.Add("Generic error");

        if (err.HasFlag(swFileSaveError_e.swReadOnlySaveError)) errors.Add("File is readonly");

        if (errors.Count == 0) errors.Add("Unknown error");
